...
import * as dcrdata from '../src/dcrdata-api'
import { DcrAddressUTXO } from '../src/types/dcrdata-api-types'

mockSochainApi.init()

const dcrdataUrl = 'https://testnet.dcrdata.org/api'
...
describe('Sochain API Test', () => {
  it('getUnspentTxs', async () => {
    ...
    const utxos: DcrAddressUTXO[] = await dcrdata.getUnspentTxs({
  ...
  it('getIsTxConfirmed', async () => {
    ...
    const isTx1Confirmed = await dcrdata.getIsTxConfirmed({
      sochainUrl: dcrdataUrl,
      network,
      hash: confirmedTxId,
    })

    const isTx2Confirmed = await dcrdata.getIsTxConfirmed({
      sochainUrl: dcrdataUrl,
      network,
  ...
  it('getConfirmedUnspentTxs', async () => {
    ...
    const utxos = await dcrdata.getConfirmedUnspentTxs({
      sochainUrl: dcrdataUrl,
      network,
      address,
    })
